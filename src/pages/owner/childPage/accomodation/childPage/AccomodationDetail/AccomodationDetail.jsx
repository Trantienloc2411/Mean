...
export default function AccommodationDetail(props) {
  const {
    roomName = "Phòng 10 - 2",
    isActive = false,
    description = "Khách sạn con nhộng trên trục giao thông liên kết với sân bay tại Sóc Sơn với quầy tiếp tân phục vụ 24 giờ/ngày",
    typeRoom = "Phòng siêu cấp",
    maxPeople = 4,
    price = "100.000",
    overPrice,
    amenities = [
        ...
        <div>
          {isActive ? (
            <Tag
              text={"Đang hoạt động"}
              textColor={"#ffffff"}
              textBackground={"#005AD2"}
            />
          ) : (
            <Tag
              text={"Tạm ngưng hoạt động"}
              textColor={"#ffffff"}
              textBackground={"#FF7369FF"}
            />
          )}
        </div>
      </div>
        ...
        <Image.PreviewGroup>
          ...
          {images.slice(0, 5).map((image, index) => (
            <div key={index} className={styles.imageContainer}>
              <Image src={image} alt={`Room view ${index + 1}`} />
