...
import { Card, Typography, Form, Space, Button, Input, Upload, message } from "antd"
...
import { useUpdateBusinessMutation, useCreateBusinessMutation } from "../../../../../../redux/services/businessApi.js"
...
const CompanyInfo = ({ companyInfo, onUpdate }) => {
  ...
  const handleEdit = () => {
    form.setFieldsValue({
      companyName: defaultData.companyName !== "Chưa có thông tin" ? defaultData.companyName : "",
      representativeName: defaultData.representativeName !== "Chưa có thông tin" ? defaultData.representativeName : "",
      representativeId: defaultData.representativeId !== "Chưa có thông tin" ? defaultData.representativeId : "",
      companyAddress: defaultData.companyAddress !== "Chưa có thông tin" ? defaultData.companyAddress : "",
      taxCode: defaultData.taxCode !== "Chưa có thông tin" ? defaultData.taxCode : "",
    })
    setIsEditing(true)
  }
  ...
  const handleSave = async () => {
    try {
      ...
      if (fileList.length > 0) {
        ...
        const { data, error } = await supabase.storage.from("business").upload(filePath, file, {
          cacheControl: "3600",
          upsert: false,
        })
      ...
      <Space direction="vertical" size="large" style={{ width: "100%" }}>
        {fields.map((field) => (
            <div key={field.name} className={styles.companyField}>
              ...
              {field.name === "file" ? (
                  defaultData.file ? (
                      <a href={defaultData.file} target="_blank" rel="noopener noreferrer">
  ...
  const renderEditForm = () => (
      <Form form={form} layout="vertical" className={styles.editForm}>
        ...
        <Form.Item name="businessLicense" label="File giấy phép kinh doanh" className={styles.uploadContainer}>
          <Upload
              listType="text"
              fileList={fileList}
          ...
          >
            <Button icon={<UploadOutlined />}>Tải lên giấy phép kinh doanh</Button>
          </Upload>
        </Form.Item>
