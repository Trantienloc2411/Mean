...
export default function Overview() {
    ...
    const { data: bookingDetail } = useGetBookingsByOwnerIdQuery(ownerDetail?.id, {
        skip: !ownerDetail?.id,
    })
    ...
    const filterDataBooking = bookingDetail?.filter((booking) => parseDate(booking.checkInHour) > new Date()) || []

    // Calculate statistics
    const totalBookings = bookingDetail?.length || 0
    const completedBookings = bookingDetail?.filter((booking) => booking.status === 7).length || 0
    const cancelledBookings = bookingDetail?.filter((booking) => booking.status === 6).length || 0
    const totalRevenue = bookingDetail?.reduce((sum, booking) => sum + (booking.totalPrice || 0), 0) || 0
    ...
    return (
        <div className={styles.contentContainer}>
            <div className={styles.pageHeader}>
                ...
                <p className={styles.pageSubtitle}>Xem thông tin đặt phòng và thống kê doanh thu</p>
            </div>
            ...
            <div className={styles.bookingOverview}>
                <CardListBooking stageLevel="1" filterDataBooking={filterDataBooking} listBookingTypeName="Đặt phòng sắp tới" />
                <CardListBooking stageLevel="2" filterDataBooking={filterDataBooking} listBookingTypeName="Đang đặt phòng" />
                <CardListBooking
                    stageLevel="3"
                    ...
                    <div className={styles.chartWrapper}>
                        <ResponsiveContainer width="100%" height={300}>
                            <AreaChart data={chartData}>
                                ...
                                <Legend />
                                <Area type="monotone" dataKey="revenue" stroke="#722ed1" fill="#f9f0ff" name="Doanh thu (VNĐ)" />
                            </AreaChart>
                        </ResponsiveContainer>
                    </div>
                </div>
