...
export default function ListBooking({
  ...
  const getPaymentMethod = (method) => {
    const methods = {
      ...
      [paymentMethodCodes.PAYOS]: "PayOS",
    };
  ...
  const statusOptions = Object.entries(bookingStatusCodes).map(
    ([key, value]) => ({
      label: (
        <span className={`${styles.statusTag} ${styles[key.toLowerCase()]}`}>
          {getBookingStatusDisplay(value)}
        </span>
      ),
  ...
  const paymentOptions = Object.entries(paymentStatusCodes).map(
    ([key, value]) => ({
      label: (
        <span className={`${styles.paymentTag} ${styles[key.toLowerCase()]}`}>
  ...
  const parseDateTime = (dateTimeStr) =>
    dayjs(dateTimeStr, "DD/MM/YYYY HH:mm:ss");
  ...
  const matchesDateTimeFilter = (booking, dateFilter) => {
    ...
    if (dateFilter.timeRange && dateFilter.timeRange.length === 2) {
      ...
      const checkInTime =
        checkInDateTime.hour() * 60 + checkInDateTime.minute(); // Convert to minutes
  ...
  const applyFilters = (filters) => {
    ...
    if (filters.dateFilter)
      filtered = filtered.filter((item) =>
        matchesDateTimeFilter(item, filters.dateFilter)
      );
  ...
  const handleFilterChange = (filterName, newValues) => {
    const updatedFilters =
      filterName === "reset"
        ? newValues
        : { ...selectedValues, [filterName]: newValues };
  ...
  const getPaymentIcon = (method) =>
    method === paymentMethodCodes.MOMO ? (
      ...
      />
    ) : method === paymentMethodCodes.PAYOS ? (
      <img
        src={
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, record) => (
        <div>
          <div className={styles.mobileValue}>
            {record._originalBooking.accommodationId.accommodationTypeId
              ?.name || "Không xác định"}
          </div>
        </div>
      ),
          ...
          <Dropdown
            menu={{
              items: [
                ...
                ...(record._originalBooking.status ===
                bookingStatusCodes.PENDING
                  ? [
                      {
                        key: "2",
            ...
            <Button icon={<FilterOutlined />} className={styles.filterButton}>
              ...
              {(Object.values(selectedValues).flat().length > 0 ||
                ...
                <span className={styles.filterBadge}>
                  {Object.values(selectedValues).flat().filter(Boolean).length +
