...
import { Dropdown, Input, Button, Menu, message, Table, Select } from "antd";
import { FilterOutlined, SearchOutlined, MoreOutlined, WalletOutlined, SortAscendingOutlined } from "@ant-design/icons";
...
export default function ListBooking({
  ...
  const { data: bookingDetailData, isLoading: isDetailLoading, isError: isDetailError } = useGetBookingByIdQuery(selectedBookingId, {
  ...
  const paymentOptions = Object.entries(paymentStatusCodes).map(([key, value]) => ({
    label: <span className={`${styles.paymentTag} ${styles[key.toLowerCase()]}`}>{getPaymentStatusDisplay(value)}</span>,
  ...
  const applyFilters = (filters) => {
    ...
    if (filters.status?.length) filtered = filtered.filter(item =>
      filters.status.includes(getBookingStatusDisplay(item._originalBooking.status)))

    if (filters.payment?.length) filtered = filtered.filter(item =>
    ...
    if (filters.searchTerm) filtered = filtered.filter(item =>
      item._originalBooking.customerId?.userId?.fullName?.toLowerCase().includes(filters.searchTerm.toLowerCase()) ||
  ...
  const applySorting = (data) => {
    ...
    switch (sortOption) {
      ...
      default:
        sortedData.sort((a, b) =>
          new Date(b._originalBooking.createdAt) - new Date(a._originalBooking.createdAt));
    }
  ...
  const getPaymentIcon = (method) => method === paymentMethodCodes.MOMO ?
    <img src={momoIcon} alt="MoMo" className={styles.paymentIcon} /> : <WalletOutlined />;
      ...
      render: (_, record) => {
        ...
        return (
          <div className={styles.timeInfo}>
            ...
            <div>{ci?.format('HH:mm')} - {co?.format('HH:mm')}</div>
          </div>
        );
      }
    },
    ...
    <div className={styles.contentContainer}>
      ...
      <div className={styles.listBooking}>
        <div className={styles.filterContainer}>
          ...
          <Dropdown
            overlay={<Filter filterGroups={filterGroups} selectedValues={selectedValues} onFilterChange={handleFilterChange} />}
            trigger={["click"]}
